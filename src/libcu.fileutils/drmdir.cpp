...
__forceinline__ int drmdir_(pipelineRedir redir, char *str) { fileutils_drmdir msg(redir, str); return msg.RC; }

int removeDir(pipelineRedir clientRedir, char *name, int f) {
	int r, r2 = 2;
	char *line;
	while (!(r = drmdir_(clientRedir, name)) && (line = strchr(name, '/')) && f) {
...
int main(int argc, char **argv) {
	...
	for (int i = parent + 1; i < argc; i++) {
		if (argv[i][0] != '-') {
			...
			if (removeDir(clientRedir, argv[i], parent)) {
